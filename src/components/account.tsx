...
const Switch: React.FC<{
    ...
    <div className={`relative flex w-full items-center rounded-lg bg-neutral-100 py-1 text-neutral-900 dark:bg-neutral-800 dark:text-white`}>
        <motion.div transition={{ type: "keyframes", duration: 0.15, ease: "easeInOut" }} animate={currentTab === 0 ? { x: 4 } : { x: "98%" }} initial={currentTab === 0 ? { x: 4 } : { x: "98%" }} className={`absolute h-5/6 w-1/2 rounded-md bg-white shadow-sm dark:bg-neutral-950`} />
        <button
            onClick={() => {
                setTab(0);
            }}
